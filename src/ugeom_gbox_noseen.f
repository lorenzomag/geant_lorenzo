...
C *** Local variables
      ...
      PARAMETER (n_det = 30)
C.
      INTEGER i, j, k, n, jm, icopy, ivol, det_mate(n_det)
C.
      REAL x, y, z, shape(10)
C.
...
C      print*, 'ugeo_finger'
C.
C.*** Definition of scintillator material (0=BGO, 1=LaBr3)
      DATA det_mate/4*0,1,1,0,0,22*1/
C.
...
C.
      ...
      shape(10) = shape(7)
C.
      CALL gsvolu('HSNB', 'PGON', 8, shape, 10, ivol)
      CALL gsatt('HSNB','SEEN',0)
C      print*, '1'
C.
C *** Make the fingers ***
C.
...
C.
      ...
      CALL gsatt('FNGR','SEEN',0)
C.
      z = -pmt_length/2.
C.
      CALL gspos('FNGR', 1, 'HSNB', 0.0, 0.0, z, 0,'ONLY') 
C.
...
C.
      ...
      CALL gsatt('SCNT','SEEN',0)
C.
      CALL gspos('SCNT', 1, 'MGOR', 0.0, 0.0, z, 0,'ONLY')
C      print*, '2
C.
C.*** Make the housing for LaBr3 scintillator and PMT
C.
      shape( 1) =   0.0
      shape( 2) = 360.0
      shape( 3) =   6.0
      ...
      shape(10) = shape(7)
C.
      CALL gsvolu('HSNL', 'PGON', 8, shape, 10, ivol)
      CALL gsatt('HSNL','SEEN',0)
C      print*, '1'
C.
C *** Make the fingers ***
C.
...
C.
      z = -pmt_length/2.
C.
      CALL gspos('FNGR', 1, 'HSNL', 0.0, 0.0, z, 0,'ONLY') 
C.
      shape( 7) = shape( 7) - d_mtl
      shape(10) = shape(10) - d_mtl
      shape( 5) = shape( 5) + d_mtl/2.
...
C.
      z = d_air(2)/2.
C.
      CALL gsvolu('SCNT', 'PGON', 25, shape, 10, ivol)           !LaBr3
      CALL gsatt('SCNT','SEEN',0)
      CALL gsatt('HSNL','COLO',3)
      CALL gsatt('HSNL','FILL',1)
C.
      CALL gspos('SCNT', 1, 'MGOR', 0.0, 0.0, z, 0,'ONLY')
C      print*, '2'
