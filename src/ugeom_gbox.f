...
C.
      ...
      shape(10) = shape(7)
C.
      CALL gsvolu('HSNB', 'PGON', 8, shape, 10, ivol) !HSNG
      CALL gsatt('HSNB','SEEN',1)
      CALL gsatt('HSNB','COLO',3)
      print*, 'HSNG shape: ', shape
C.
C *** Make the fingers ***
C.
...
C.
      CALL gsvolu('FNGB', 'PGON',  6, shape, 10, ivol) !FNGR
      CALL gsatt('FNGB','SEEN',0)
      print*, "FNGR shape = ", shape
C.
      z = -pmt_length/2.
C.
      print*, "FNGR pos within HSNG ", x, y, z
      CALL gspos('FNGB', 1, 'HSNB', 0.0, 0.0, z, 0,'ONLY') 
C.
...
C.
      z = d_mtl/2.
C.
      print*, "MGOR shape ", shape
      print*, "MGOR pos within FNGR = ", x, y, z
      CALL gsvolu('MGOB', 'PGON', 16, shape, 10, ivol) !MGOR
      CALL gsatt('MGOB','SEEN',0)
C.
      CALL gspos('MGOB', 1, 'FNGB', 0.0, 0.0, z, 0,'ONLY')
C.
...
C.
      z = d_air(2)/2.
C.
      print*, "SCNT pos within MGOR = ", x, y, z
      CALL gsvolu('SCNB', 'PGON', n_detmate, shape, 10, ivol)
      CALL gsatt('SCNB','SEEN',0)
C.
      CALL gspos('SCNB', 1, 'MGOB', 0.0, 0.0, z, 0,'ONLY')
C      print*, '2
C.
...
C.
      ...
      CALL gsatt('HSNL','SEEN',1)
      CALL gsatt('HSNL','COLO',2)
C      print*, '1'
C.
C *** Make the fingers ***
C.
      ...
      shape(10) = shape(10) - air_gap/2.
C.
      CALL gsvolu('FNGL', 'PGON',  6, shape, 10, ivol)
      CALL gsatt('FNGL','SEEN',0)
C.
      z = -pmt_length/2.
C.
      CALL gspos('FNGL', 1, 'HSNL', 0.0, 0.0, z, 0,'ONLY') 
C.
      shape( 7) = shape( 7) - d_mtl
      shape(10) = shape(10) - d_mtl
      shape( 5) = shape( 5) + d_mtl/2.
...
C.
      z = d_mtl/2.
C.
      CALL gsvolu('MGOL', 'PGON', 16, shape, 10, ivol)
      CALL gsatt('MGOL','SEEN',0)
C.
      CALL gspos('MGOL', 1, 'FNGL', 0.0, 0.0, z, 0,'ONLY')
C.
      shape( 7) = shape( 7) - d_air(1)
      shape(10) = shape(10) - d_air(1)
      shape( 5) = shape( 5) + d_air(2)/2.
...
C.
      z = d_air(2)/2.
C.
      print*, "SNCT dimentions: ", shape
      CALL gsvolu('SCNL', 'PGON', 25, shape, 10, ivol)           !LaBr3
      CALL gsatt('SCNL','SEEN',0)
C.
      CALL gspos('SCNL', 1, 'MGOL', 0.0, 0.0, z, 0,'ONLY')
C      print*, '2'
...
C.
      z = depth/2.
C.
      CALL gspos('PMT ', 1, 'HSNB', 0.0, 0.0, z, 0, 'ONLY') 
      CALL gspos('PMT ', 2, 'HSNL', 0.0, 0.0, z, 0, 'ONLY')
C.
      RETURN
      END
C.
...
C.
      name = 'HSNB'
      CALL gsdet('SCNB',name,1,name,nbitsv,idtyp,100,0,iset,idet)
      CALL gsdeth('SCNB',name,5,chnamh,nbitsh,orig,fact)
...
************************************************************************
*                                                                      *
*                          Detector Scintillator LaBr3		       *
*                                                                      *
************************************************************************
C.
      idtyp = 4      !idtyp 3 is DSSSD
C.
      nbitsv = 10
C.
      name = 'HSNL'
