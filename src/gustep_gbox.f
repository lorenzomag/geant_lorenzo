...
C.
      If(in_new_vol.eq.1.and.ivert.eq.1)then
        If(chname_nlevel.eq.'FNGB'.or.chname_nlevel.eq.'FNGL'.or.
     &     chname_nlevel.eq.'SCNB'.or.chname_nlevel.eq.'SCNL')then
          If(ipart.eq.1)then
...
C.
      If(in_new_vol.eq.1.and.itrtyp.eq.7)then
        If(chname_nlevel.eq.'PMT ')then
          CALL ghipmt
          goto 200
        Elseif(chname_nlevel.ne.'SCNB'.and.chname_nlevel.ne.'SCNL'.and.
     &         chname_nlevel.ne.'MGOB'.and.chname_nlevel.ne.'MGOL')then
          istop = 3
          goto 200
        Endif
      Endif
...
C.
      If(numed.eq.n_detmate.and.itrtyp.ne.7)then
        CALL ghidet
      ENDIF
      If(numed.eq.25.and.itrtyp.ne.7)then
        CALL ghidet
      ENDIF
C.
C.
